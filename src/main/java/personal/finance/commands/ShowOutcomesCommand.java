...
public class ShowOutcomesCommand extends Command {

    public ShowOutcomesCommand (ITransactionService transactionService) {
        super("Spenderande pengar", "Visar spenderande pengar (årsvis, månadsvis, veckovis och dagvis)", transactionService);
    }

    @Override
    public void execute() {
        System.out.print("Skriv en tidsperiod (dag, vecka, månad, år): ");
        String outcomePeriod = scanner.nextLine().toLowerCase();
        List<Transaction> filterOutcome = new ArrayList<>();
        List<Transaction> transactions = transactionService.getAllTransactions();

        switch (outcomePeriod) {
            case "dag":
        ...
        if (filterOutcome.isEmpty()) {
            System.out.println("Inga utgifter att visa under denna period.");
        } else {
            System.out.println("Utgifter för denna period: ");
            for (Transaction t : filterOutcome) {
                printTransactions(t, outcomePeriod);
            }
        }
    }
}
